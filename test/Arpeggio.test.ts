...
@suite class ArpeggioTests {
    @test 'createNote returns null if no chord passed in'() {
        const arpNote = new ArpeggioNote(0, 1, c => c.getPitchByIndex(0), 80);
    ...
    @test 'createNote returns new note object'() {
        const chord = new Chord().addPitches([4, 8, 11]);
        const arpNote = new ArpeggioNote(0, 1, c => c.getPitchByIndex(1), 81, 3);
    ...
    @test 'createNote takes percent for setting initial velocity tween value'() {
        const chord = new Chord().addPitches([4, 8, 11]);
        const arpNote = new ArpeggioNote(0, 1, c => c.getPitchByIndex(1), Tween.linear(0, 80), 3);
    ...
    @test 'getNotesStartingInRange handles wrapping around start/end'() {
        ...
        arp.notes.push(
            new ArpeggioNote(0, 1, c => c.getPitchByIndex(0), 80),
            new ArpeggioNote(1, 1, c => c.getPitchByIndex(1), 81),
            new ArpeggioNote(2, 1, c => c.getPitchByIndex(2), 82),
            new ArpeggioNote(3, 1, c => c.getPitchByIndex(1), 83)
        );
    ...
    @test 'addNote can add new note'() {
        const arp = new Arpeggio(4);
        arp.addNote(0, 1, c => c.getPitchByIndex(0), 80);
    ...
    @test 'addNote can take array of note starts'() {
        const arp = new Arpeggio(4)
            .addNote([0,1,2,3], 1, c => c.getPitchByIndex(0), 80);
        expect(arp.notes.length).to.equal(4);
    }

    @test 'addNote can take velocity tween'() {
        const arp = new Arpeggio(4)
            .addNote(0, 1, c => c.getPitchByIndex(0), Tween.linear(1, 127));
    }
}